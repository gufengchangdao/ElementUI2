 ...
 * 下拉列表为表格形式的 ComboBox
 */
public class DropdownTableComboBoxTest extends JPanel {
	private DropdownTableComboBoxTest() {
		super(new BorderLayout());
		java.util.List<java.util.List<Object>> aseries = new ArrayList<>();
		...
		DropdownTableComboBox<java.util.List<Object>> combo = new DropdownTableComboBox<>(aseries, model);
		...
		combo.addItemListener(e -> {
			if (e.getStateChange() == ItemEvent.SELECTED) {
				java.util.List<Object> rowData = combo.getSelectedRow();
				wtf.setText(Objects.toString(rowData.get(1)));
				htf.setText(Objects.toString(rowData.get(2)));
			}
		});
		ListCellRenderer<? super java.util.List<Object>> renderer = combo.getRenderer();
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(DropdownTableComboBoxTest::createAndShowGui);
	}
	...
	private static void createAndShowGui() {
		...
		frame.getContentPane().add(new DropdownTableComboBoxTest());
		frame.pack();
...
class DropdownTableComboBox<E extends java.util.List<Object>> extends JComboBox<E> {
	...
	protected DropdownTableComboBox(java.util.List<E> list, DefaultTableModel model) {
		super();
