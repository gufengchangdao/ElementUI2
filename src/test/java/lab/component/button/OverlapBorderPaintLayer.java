 ...
 * 不同形状的按钮组
 */
public class OverlapBorderPaintLayer extends JPanel {
	private OverlapBorderPaintLayer() {
		super(new BorderLayout());
		java.util.List<String> list = Arrays.asList("aaa", "bb", "c");
	...
	public static JPanel makeBreadcrumb(java.util.List<String> list, Color color, int overlap) {
	...
	public static JPanel makeChevronBreadcrumb(java.util.List<String> list, Color color, int overlap) {
		JPanel p = makePanel(overlap);
		p.setBorder(BorderFactory.createEmptyBorder(5, overlap + 5, 5, 5));
		ButtonGroup bg = new ButtonGroup();
		list.forEach(title -> {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(() -> {
			...
			p.add(new OverlapBorderPaintLayer());
			SwingTestUtil.test();
		});
	}
}
