...
public class JidePopupMenu extends JPopupMenu implements Scrollable {
	...
	protected Point getPopupMenuOrigin(Component invoker, int x, int y) {
		...
		Rectangle bounds = UIUtil.ensureOnScreen(new Rectangle(p, size));
	...
	public void setLocation(int x, int y) {
		...
		if (isVisible() && y <= 0) {
			// cannot call setLocation because it will be recursive call. So call deprecated move in order to bypass this.
			setLocation(x, y);
		} else {
	...
	public Dimension getPreferredScrollableViewportSize() {
		...
		Dimension screenSize = UIUtil.getScreenSize(this);
