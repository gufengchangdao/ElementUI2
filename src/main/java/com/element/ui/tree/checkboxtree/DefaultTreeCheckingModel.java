...
public class DefaultTreeCheckingModel implements TreeCheckingModel {
	...
	public TreePath[] getCheckingPaths() {
		return checkedPathsSet.toArray(new TreePath[0]);
	}
	...
	private Vector<TreePath> getCheckingRoots(TreePath path) {
		Object node = path.getLastPathComponent();
		Vector<TreePath> roots = new Vector<>();
	...
	public TreePath[] getGreyingPaths() {
		return greyedPathsSet.toArray(new TreePath[0]);
	}
	...
	private String toString(TreePath path) {
		...
		StringBuilder description = new StringBuilder("Path checked: " + checkString + " greyed: " + greyString + " enabled: " + enableString + " Name: "
				+ path.toString() + "\n");
		for (TreePath childPath : getChildrenPath(path)) {
			description.append(toString(childPath));
		}
