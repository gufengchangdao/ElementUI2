...
public class CheckBoxTreeSelectionModel extends DefaultTreeSelectionModel implements TreeModelListener {
	...
	/**
	 * Used in {@link #areSiblingsSelected(TreePath)} for those paths pending added so that they are not
		...
		try {
			if (needCheckPathSelection) {
				...
				if (toBeRemoved.size() > 0) {
					delegateRemoveSelectionPaths(toBeRemoved.toArray(new TreePath[0]));
				}
			} else {
				// deselect all descendants of paths[]
				List<TreePath> toBeRemoved = new ArrayList<>();
		...
		try {
			...
			for (TreePath path : paths) {
				...
				while (!stack.isEmpty()) {
					...
					for (int i = 0; i < childCount; i++) {
						Object childNode = getChild(node, i);
						if (!CompareUtil.equals(childNode, peekNode)) {
				...
				if (toBeAdded.size() > 0) {
					delegateAddSelectionPaths(toBeAdded.toArray(new TreePath[0]));
				}
	...
	public void setBatchMode(boolean batchMode) {
		...
		if (!_batchMode) {
			TreePath[] treePaths = _toBeRemoved.toArray(new TreePath[0]);
			_toBeRemoved.clear();
			super.removeSelectionPaths(treePaths);
			treePaths = _toBeAdded.toArray(new TreePath[0]);
			_toBeAdded.clear();
			super.addSelectionPaths(treePaths);
		}
	}

	private Set<TreePath> _toBeAdded = new HashSet<>();
	private Set<TreePath> _toBeRemoved = new HashSet<>();
