...
public class CheckBoxTree extends JTree {
	...
	protected void init() {
		...
		ListenerUtil.insertMouseListener(this, handler, 0);
	 ...
	 * using {@link #setCellRenderer(TreeCellRenderer)}.
	 *
	...
	private static class CellRendererPropertyChangeListener implements PropertyChangeListener, Serializable {
		...
		private void toggleSelections(TreePath[] treePaths) {
			...
			try {
				if (pathToAdded.size() > 0) {
					selectionModel.addSelectionPaths(pathToAdded.toArray(new TreePath[0]));
				}
				if (pathToRemoved.size() > 0) {
					selectionModel.removeSelectionPaths(pathToRemoved.toArray(new TreePath[0]));
				}
			} finally {
