...
//		int columnMargin = header.getColumnModel().getColumnMargin();
		...
		GroupableColumnModel.IColumnGroup draggedGroup = null;
...
//		TableCellRenderer renderer = getRenderer(aColumn);
	...
	protected void paintCell(Graphics g, Rectangle cellRect, GroupableColumnModel.IColumnGroup group) {
		...
		Component component = renderer.getTableCellRendererComponent(header.getTable(), group.getTitle(), false, hasFocus, -1, -1);
	...
	protected Dimension getGroupSize(GroupableColumnModel.IColumnGroup group) {
		...
		TableColumnModel columnModel = header.getColumnModel();
		if (columnModel instanceof GroupableColumnModel groupModel) {
			TableCellRenderer renderer = getRenderer(group);
	...
	protected void scrollToColumn(int col) {
		...
		if ((header.getParent() == null) || ((container = header.getParent().getParent()) == null) || !(container instanceof JScrollPane) || ((table = header.getTable()) == null)) {
			return;
		}
	...
	protected boolean canResize(Point p, TableColumn column, JTableHeader header) {
		Rectangle bounds = getGroupHeaderBoundsFor(column);

		return (bounds != null && !bounds.contains(p)) && (column != null) && header.getResizingAllowed() && column.getResizable();
	}
	...
	public Rectangle getGroupHeaderBoundsFor(GroupableColumnModel.IColumnGroup group) {
		Rectangle bounds = new Rectangle();
		...
		public void mousePressed(MouseEvent e) {
			...
			if (index != -1) {
				...
				if (!clickedGroupHeader) {
					...
					GroupableColumnModel.IColumnGroup columnGroup = groupModel.getColumnGroupFor(column);
...
//				int newColumnIndex = columnIndex + (headerLeftToRight ? direction : -direction);
				...
				GroupableColumnModel.IColumnGroup newGroup = groupModel.getColumnGroupFor(groupModel.getColumn(newColumnIndex));
				if (newGroup != null) {
				...
				if (Math.abs(draggedDistance) > (width / 2)) {
					int selectedIndex = convertColumnIndexToModel(header.getColumnModel(), getSelectedColumnIndex());
					...
					selectColumn(convertColumnIndexToView(header.getColumnModel(), selectedIndex), false);
				}
				...
				if (resizingColumn != null) {
					...
					if (shouldMove) {
						if (groupModel != null && currentGroup == null) {
							GroupableColumnModel.IColumnGroup newGroup;
						...
						if (shouldMove) {
							if (0 <= newColumnIndex && newColumnIndex < cm.getColumnCount()) {
								...
								if (Math.abs(draggedDistance) > (width / 2)) {
									...
									int selectedIndex = convertColumnIndexToModel(header.getColumnModel(), viewIndex);
