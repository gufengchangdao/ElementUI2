	...
	protected void paintTabBackground(Graphics g, int tabPlacement, int tabIndex, int x, int y, int w, int h, boolean isSelected) {
		...
		if (isSelected) {
			...
			if (tabPane.isEnabled() && tabPane.isEnabledAt(tabIndex)) {
				GradientPaint gradientShadowTmp = new GradientPaint(0, 0, new Color(255, 255, 200), 0, y + h / 2f, new Color(240, 255, 210));
				g2D.setPaint(gradientShadowTmp);
			} else {
				GradientPaint gradientShadowTmp = new GradientPaint(0, 0, new Color(240, 255, 210), 0, y + 15 + h / 2f, new Color(204, 204, 204));
				g2D.setPaint(gradientShadowTmp);
			}
		}
		g2D.fill(shape);
