...
/**
 * JideBoxLayout 与 BoxLayout 非常相似，所有组件的排列方式都是从左到右或从上到下。与 BoxLayout 不同的是，在向该布局添加组件时存在三种可能的约束 - FIX、FLEXIBLE 和 VARY。
 *
 * <ul>
 *     <li>FIX: 使用组件的首选尺寸并且尺寸是固定的
 *     <li>FLEXIBLE: 尊重组件的首选大小，但可以更改大小。
 *     <li>VARY: 忽略首选尺寸。它的大小是根据剩余的面积计算的。
 * </ul>
 * <p>
 * 这是 {@link JideSplitPane} 的默认布局管理器
 */
public class JideBoxLayout implements LayoutManager2, Serializable {
	...
	public void addLayoutComponent(Component comp, Object constraints) {
		_constraintMap.put(comp, Objects.requireNonNullElse(constraints, FLEXIBLE));
		layoutReset();
	}
	...
	/**
	 * Checks of the layout should be reset when {@link #invalidateLayout(Container)} is called.
	 *
	...
	/**
	 * Sets the flag if the layout should be reset when {@link #invalidateLayout(Container)} is called.
	 *
