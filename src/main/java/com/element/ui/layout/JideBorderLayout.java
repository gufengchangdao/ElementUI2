...
/**
 * 这是BorderLayout的修改版本。与BorderLayout不同的是，TOP 和 BOTTOM 组件的宽度与 CENTER 组件的宽度相同。而在 BorderLayout 中，它们的宽度与容器的宽度相同。
 *
 * @see BorderLayout
 */
	...
	public void addLayoutComponent(String name, Component comp) {
		synchronized (comp.getTreeLock()) {
			...
			switch (name) {
				...
				case "East" -> east = comp;
				case "West" -> west = comp;
				case BEFORE_FIRST_LINE -> firstLine = comp;
				case AFTER_LAST_LINE -> lastLine = comp;
				case BEFORE_LINE_BEGINS -> firstItem = comp;
				case AFTER_LINE_ENDS -> lastItem = comp;
