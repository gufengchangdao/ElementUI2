 ...
 * change it using {@link #setHighlightColor(Color)}.
 * <p/>
...
public class TextComponentSearchable extends Searchable implements DocumentListener, PropertyChangeListener {
	...
	protected void addHighlight(final int index, final String text, boolean incremental) throws BadLocationException {
		if (_component instanceof final JTextComponent textComponent) {
			...
			if (!incremental) {
				Runnable runnable = () -> scrollTextVisible(textComponent, index, text.length());
	...
	public int findFromCursor(String s) {
		...
		if (_component instanceof JTextComponent) {
			...
			if (found == -1) {
				found = isCaseSensitive() ? text.indexOf(s) : indexOf(text, s, 0);
	...
	public int reverseFindFromCursor(String s) {
		...
		if (_component instanceof JTextComponent) {
			...
			if (found == -1) {
				found = isCaseSensitive() ? text.lastIndexOf(s) : lastIndexOf(text, s, text.length() - 1);
				if (found <= selectedIndex) {
					found = -1;
				}
			}
	...
	private static class HighlighCache extends HashMap {
