...
//        map.setParent(audioMap);
	...
	protected void paintMenuItem(Graphics g, JComponent c,
		...
		// Paint the Check
		if ((c.getUIClassID().contains("CheckBoxMenu") || c.getUIClassID().contains("RadioButtonMenu")) && checkIcon != null) {
	...
	private class PropertyChangeHandler implements PropertyChangeListener {
		public void propertyChange(PropertyChangeEvent e) {
			...
			switch (name) {
				...
				case "opaque":
				case AbstractButton.CONTENT_AREA_FILLED_CHANGED_PROPERTY:
					if (e.getSource() instanceof JMenuItem lbl) {
						lbl.repaint();
					}
					break;
				case "text":
...
//        // Visual feedback
	...
	protected boolean isDownArrowVisible(Container c) {
		if (c instanceof TopLevelMenuContainer && ((TopLevelMenuContainer) c).isMenuBar()) {
			...
			return true;
		} else return !(c instanceof JMenuBar);
	}
}
