...
public abstract class AbstractIntelliHints implements IntelliHints {
	 ...
	 * to accept the hint in a different way. For example, {@link FileIntelliHints} will append the
	 ...
	 * @see #removeShowHintsKeyStroke(KeyStroke)
	...
	public KeyStroke[] getAllShowHintsKeyStrokes() {
		...
		return _showHintsKeyStrokes.toArray(new KeyStroke[0]);
	}
			...
			if (hints instanceof AbstractIntelliHints aih) {
				if (tf.isEnabled()) {
					if (aih.isHintsPopupVisible()) {
						...
						if (key != null) {
							Action action = aih.getDelegateComponent().getActionMap().get(key);
							if (action != null) {
